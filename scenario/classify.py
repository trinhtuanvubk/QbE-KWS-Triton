...
arr = ['yes', 'no', 'up', 'down', 'left', 'right', 'on', 'off', 'stop', 'go', 'unknown', 'silence']
...
def classify(model, args, audio, labels_list):
    # print(model)
    feature = models.MelSpectrogram(args).feature(audio)
    print("feature",feature.shape)
    # print(feature)
    ...
    output_np =  output.cpu().detach().numpy()
    # print(output_np)
    output = softmax_(output_np)
    print(output.shape)
    # probs = np.max(output,axis=-1)
    # print(probs.shape)
    argmax_idx = np.argmax(output, axis=-1, keepdims=True)
    print(list(argmax_idx))
    # print(output)
    label = [labels_list[i] for i in list(argmax_idx)]
    probs = [output[i] for i in list(argmax_idx)]
    return label, probs
...
if __name__=="__main__":
    ...
    # x= torch.cat((pad_audio,pad_audio), 0)
    x = pad_audio
    print("pad:",x)
    print(x.shape)
    ...
    label, probs = classify(model, args, x, arr)
    print(label, probs)
